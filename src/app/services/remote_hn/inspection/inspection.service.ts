...
export class InspectionService {
  ...
  public loadInspectionsV1(document: string, plate: string, policy: string) {
    ...
    return Observable.create((observer) => {
      this.httpClient
        .post("/Info/DatosCliente", body, false)
        .subscribe((data) => {
  ...
  public loadInspections(document: string, plate: string, policy: string) {
    ...
    return Observable.create((observer) => {
      this.httpClient
        .post("/Info/DatosClienteV2", filteredBody, false)
        .subscribe((data) => {
          if (data.status && data.data !== null) {
  ...
  public loadInspectionDetails(quoteNumber: string) {
    ...
    return Observable.create((observer) => {
      this.httpClient
        .post("/Info/Cotizacion", body, false)
        .subscribe((data) => {
          if (data.status && data.data !== null) {
  ...
  public uploadInspection(body: any) {
    ...
    return Observable.create((observer) => {
      this.httpClient
        .post("/Inspeccion/EnvioRespuesta", body, false)
        .subscribe((data) => {
          observer.next(data);
          observer.complete();
        });
  ...
  public updateInspection(body: any) {
    ...
    return Observable.create((observer) => {
      this.httpClient
        .post("/Inspeccion/ActualizacionDatosVehiculo", request, false)
        .subscribe((data) => {
          observer.next(data);
          observer.complete();
        });
